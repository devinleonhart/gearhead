...
const rollOpposed = function(playerDicePool, playerLimit, opponentDicePool, opponentLimit) {
  const rolledPlayerDice = new Array(playerDicePool).fill().map(() => Math.round(Math.random() * (6 - 1)) + 1).sort((a, b) => a - b);
  const rolledOpponentDice = new Array(opponentDicePool).fill().map(() => Math.round(Math.random() * (6 - 1)) + 1).sort((a, b) => a - b);
  ...
  if(rolledPlayerDice.filter((num) => num === 1).length >= Math.ceil(playerDicePool / 2)) {
    if(hits === 0) {
      result += " + Critical Glitch";
      reaction = ":scream_cat: :skull:";
    }
...
const availability = function(availability, cha, negotiate, limit) {
  return `
  Availability Roll!
  Rolling with CHA + Negotiate: ${cha + negotiate} with a limit of ${limit} vs Availability: ${availability} with no limit.
  ${rollOpposed((cha+negotiate),limit,availability,999)}
  `;
};

const availabilityWithContact = function(availability, cha, negotiate, connection) {
  return `
  Availability Roll with Contact!
  Rolling with CHA + Negotiate: ${cha + negotiate} limit of ${3+connection} vs Availability: ${availability} with no limit.
